...
async function rotate(
  ...
  if (nextFp === previous.fingerprint) {
    console.error('\n  stripe login completed but produced the same key Capy already had.');
    console.error('  This usually means login was cancelled. Re-run to try again.');
    process.exit(1);
  }
