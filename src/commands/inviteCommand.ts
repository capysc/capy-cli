...
export interface InviteOpts {
  ...
  web?: boolean;
  /**
   * CAP-529, additive and off by default: mint a v3 (stored-blob) short code
...
export class InviteCommand {
  ...
  async execute(email: string, opts: InviteOpts = {}): Promise<void> {
    ...
    try {
      ...
      //    v3 (CAP-529, additive — gated on opts.v3): T shrinks to 12 bytes
      ...
      if (webParams) {
        ...
        console.log('');
        // v3 (CAP-529): the code is bare T, not a blob-in-code — the prior
