...
export class OrgCommand {
  ...
  private async _execute(): Promise<void> {
    ...
    const currentOrgId = projectState.organizationId;

    // Authenticate silently using cached session — no OAuth
    ...
    if (orgs.length <= 1) {
      console.log(`\n  Organization: ${B(currentOrg?.name || currentOrgId)}`);
    ...
    if (!hasOrgKey(selectedOrg.id, authResult.user_id!)) {
      console.log(`\n  No master key found for ${B(selectedOrg.name)}.`);
      console.log(`  Run ${B('capy')} in a project directory to set up this organization.\n`);
      return;
    }
