...
program
  ...
  .action(async (options, cmd) => {
    if (cmd.args.length > 0) {
      ...
      console.log(`    ${B('capy-dev')} setup --json           \x1b[90mPlan/apply first-run setup as JSON (no TTY)\x1b[0m`);
      console.log(`    ${B('capy-dev')} sync --json            \x1b[90mSync an already-initialized project as JSON (no TTY)\x1b[0m`);
...
program
  ...
  .action(async (branch, options, command) => {
    ...
    if (hosted || (mergedOptions.json && !mergedOptions.create)) {
      ...
      process.exit(await runCheckoutJsonCommand(branch, { ...mergedOptions, nonTty: hosted }, true));
    }
...
program
  ...
  .action(async (options: Readonly<{
    ...
    // Same duplicated root/subcommand `--json` binding as in src/index.ts.
    const json = command.optsWithGlobals().json === true;
    if (options.plan || options.confirm) {
      ...
      process.exit(await runPushJsonCommand({ plan: options.plan, confirm: options.confirm,
        expectedUserId, serviceOrigin: options.serviceOrigin }, true));
    }
    if (json) {
...
program
  ...
  .action(async (options, command) => {
    ...
    const cmd = new StatusCommand(false, true, expectedUserIdFor(command));
...
flow
  ...
  .action(async (id: string, options: import('./commands/flowSetupCommand').FlowSetupOptions, command) => {
    ...
    const code = await runFlowSetupCommand(id, { ...options, expectedUserId }, true);
    if (code !== 0) process.exit(code);
  });
...
flow
  ...
  .action(async (id: string, options: import('./commands/flowAddCommand').FlowAddOptions, command) => {
    ...
    const code = await runFlowAddCommand(id, { ...options, expectedUserId }, true);
    if (code !== 0) process.exit(code);
  });
...
flow
  ...
  .action(async (id: string, options: { json?: boolean; yes?: boolean; nonTty?: boolean }, command) => {
    ...
    const cmd = new FlowCancelCommand(process.env.CAPY_API_URL, true);
...
program
  ...
  .action(async (_opts: any, cmd: any) => {
    ...
    const code = expectedUserId !== undefined
      ? await (await import('./commands/runHostedCommand')).runHostedCommand(childArgs, expectedUserId, true)
      : await runCommand(childArgs, true);
    process.exit(code);
  });
...
program
  ...
  .action(async (varName, options, command) => {
    ...
    if (options.checkReadiness) {
      ...
      const readiness = await inspectLocalRotateReadiness({ ...rotateOptions, devMode: true });
      console.log(JSON.stringify(readiness));
      return;
    }
    if (globals.flow) {
      const { runRotateFlow } = await import('./commands/rotateFlow');
      await runRotateFlow(varName, rotateOptions, true);
      return;
    }
    const { RotateCommand } = await import('./commands/rotateCommand');
    const execute = () => new RotateCommand(true).execute(varName, rotateOptions);
    if (globals.json) {
      const { runWithInteraction, createJsonLineInteraction } = await import('./ui/interaction');
