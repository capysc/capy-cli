/**
 * Generic CLI-side relay for a `payload-both` keep screen (W2-A) — the
 * pattern every future "migrate a loopback screen with a CLI->page and
 * page->CLI payload" effort should reuse rather than re-deriving.
 *
 * Sequence, all on one connection: create -> print/open the keep URL ->
 * wait for the page to attach and publish `page_pubkey` -> seal and send
 * the request payload over the broker reverse channel -> wait for the
 * page's sealed answer -> typed-validate it. Mirrors the no-submit relay
 * (`authService.ts`'s `relayAuthScreenViaKeep`) in spirit: every ending is
 * a coded variant, NEVER a throw, and the caller's contract is exactly the
 * same one CAP-376 established — "fall back to the loopback transport on
 * anything but `answered`". Nothing here ever logs or returns the
 * plaintext request/answer payload; only the caller (who minted them and
 * knows what they contain) does that.
 */
