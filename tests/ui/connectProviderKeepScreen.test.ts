/**
 * `capy connect` (provider picker) over the broker reverse channel (W2-C).
 * Mirrors branchListKeepScreen.test.ts/secretIntakeKeepScreen.test.ts (W2-A):
 * the CLI sends a sealed ConnectProviderData request once the page publishes
 * page_pubkey, opens the page's sealed `{provider}` answer (Wizard's
 * `{__action:'submit', ...answer()}` shape), and resolves it against the
 * connector list THIS run offered.
 *
...
import { chooseConnectorInBrowser } from '../../src/ui/connectScreens';
...
const CONN_ID = 'conn-connect-provider-1';
...
const CONNECTORS = [{ id: 'stripe', description: 'Stripe secret keys' }];

describe('CAPY_KEEP_SCREENS=1', () => {
  test('sends the sealed ConnectProviderData request, opens the sealed answer, resolves the picked provider', async () => {
    ...
    const promise = chooseConnectorInBrowser({
      projectName: 'demo',
      branch: 'main',
    ...
    try {
      ...
      if (wire.requestCiphertext) {
        ...
        wire.answerCiphertext = await sealEnvelopePageSide({
          plaintext: JSON.stringify({ __action: 'submit', provider: 'stripe' }),
          connectionId: CONN_ID,
          clientPubkeyB64: wire.createBody!.client_pubkey as string,
        });
      }
    ...
    expect(requestData).toMatchObject({ nonce: '', connectors: [{ id: 'stripe' }] });
    expect(wire.createBody?.purpose).toBe('connect-provider');
    expect(result).toEqual({ provider: 'stripe', cancelled: false });
  });
});

describe('flag unset (default)', () => {
  test('chooseConnectorInBrowser with authService supplied still takes the loopback path — zero broker traffic, unchanged', async () => {
    delete process.env.CAPY_KEEP_SCREENS;
    let brokerHit = false;
    globalThis.fetch = ((url: any, init?: any) => {
    ...
    const res = await fetch(`http://127.0.0.1:${u.port}/submit`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ nonce, payload: { provider: 'stripe' } }),
    });
