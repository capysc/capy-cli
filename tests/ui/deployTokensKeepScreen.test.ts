/**
 * `capy deploy list`/`revoke` over the broker reverse channel (W2-C).
 * Mirrors branchListKeepScreen.test.ts (W2-A/W2-C): the CLI sends a sealed
 * DeployTokensData request once the page publishes page_pubkey, opens the
 * page's sealed `{action:'revoke', deployId}` answer, and resolves it
 * against the token list THIS run offered.
 *
...
import { showDeployTokensInBrowser } from '../../src/ui/deployScreens';
...
const CONN_ID = 'conn-deploy-tokens-1';
...
const TOKENS = [
  { deployId: 'capyfake0001deadbeef', label: 'ci', createdAge: '2 days ago', createdOn: '2026-08-09', revokedAge: null },
];

describe('CAPY_KEEP_SCREENS=1', () => {
  test('sends the sealed DeployTokensData request, opens the sealed answer, resolves the revoke', async () => {
    ...
    const promise = showDeployTokensInBrowser({
      projectName: 'demo',
      tokens: TOKENS,
      open: false,
    ...
    try {
      ...
      if (wire.requestCiphertext) {
        ...
        wire.answerCiphertext = await sealEnvelopePageSide({
          plaintext: JSON.stringify({ action: 'revoke', deployId: 'capyfake0001deadbeef' }),
          connectionId: CONN_ID,
          clientPubkeyB64: wire.createBody!.client_pubkey as string,
        });
      }
    ...
    expect(requestData).toMatchObject({ nonce: '', tokens: [{ deployId: 'capyfake0001deadbeef' }] });
    expect(wire.createBody?.purpose).toBe('deploy-tokens');
    expect(result).toEqual({ deployId: 'capyfake0001deadbeef', cancelled: false });
  });
});

describe('flag unset (default)', () => {
  test('showDeployTokensInBrowser with authService supplied still takes the loopback path — zero broker traffic, unchanged', async () => {
    delete process.env.CAPY_KEEP_SCREENS;
    let brokerHit = false;
    globalThis.fetch = ((url: any, init?: any) => {
    ...
    const res = await fetch(`http://127.0.0.1:${u.port}/submit`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ nonce, payload: { action: 'revoke', deployId: 'capyfake0001deadbeef' } }),
    });
